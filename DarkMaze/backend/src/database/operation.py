...
def create_user(username):
    ...
    try:
        cursor.execute(
            """
            INSERT INTO game_state (username, current_level_name,
            map_size, health, path, current_position)
            VALUES (?, ?, ?, ?, ?, ?)
        """,
            (
                username,
...
def reset_game_state(username):
    ...
    if result:
        ...
        cursor.execute(
            """
            UPDATE game_state
            SET current_level_name = ?, map_size = ?, health = ?,
...
def save_game_state(
    ...
    if result:
        ...
        cursor.execute(
            """
            UPDATE game_state
            SET current_level_name = ?, map_size = ?,
            health = ?, path = ?, current_position = ?
            WHERE username = ?
        """,
            (
                current_level_name,
