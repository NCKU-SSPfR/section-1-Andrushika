...
from .database.operation import create_user, get_latest_game_state, \
                                reset_game_state
...
async def login(request: Request, response: Response):
    ...
    if username == "":
        ...
        return JSONResponse(
            {"message": "Username is empty", "cookies": [], "status": 0}
        )
    ...
    return JSONResponse(
        {"message": "Login successful", "cookies": [cookie], "status": 1}
    )
