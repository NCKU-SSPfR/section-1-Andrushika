...
def _parse_map(map_string, map_size, reversal_nodes=[]):
    ...
    for Q in QQ:
        ...
        game_map.extend([first_half % 2, second_half % 2])

    while len(game_map) < width * height:
        game_map.append(0)

    game_map = game_map[: width * height]

    swiper = np.array(game_map).reshape((height, width))
...
def _load_maze_from_json(maze_level_name):
    with open(
        "./src/game/maze_level/" + maze_level_name + ".json",
        "r",
        encoding="utf-8"
    ) as f:
        data = json.load(f)
...
def hit_obstacle(position, maze_level_name):
    x, y = position
    maze_data = _load_maze_from_json(
        maze_level_name
    )  # You can replace this with the actual level you're working with
    grid = maze_data["map"]

    # Check if the position is within the bounds of the grid
    if 0 <= x < grid.shape[0] and 0 <= y < grid.shape[1]:
        # Return True if there's an obstacle (1) at the position
        # False if free space (0)
        return grid[y, x] == 1
    else:
